...
import { platforms } from "@/components/dashboard/data";
...
export function useSprints() {
    ...
    const handleBulkUploadTasks = bulkUploadWrapper(async (tasks: BulkTask[]) => {
        ...
        const newTickets: Ticket[] = uniqueNewTickets.map(task => {
            ...
            const canonicalPlatform = platforms.find(t => t.value.toLowerCase() === task.platform.toLowerCase())?.value || 'Web';
    ...
    const handleBulkLogProgress = bulkUploadWrapper(async (logs: BulkProgressLog[]) => {
        ...
        for (const log of logs) {
            ...
            if (!ticket) {
                ...
                const canonicalPlatform = platforms.find(t => t.value.toLowerCase() === log.platform!.toLowerCase())?.value || 'Web';
